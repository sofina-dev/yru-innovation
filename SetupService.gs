...
function seedDemoSubmissions() {
  ...
  const records = DEMO_SUBMISSIONS_.map(function (demo, index) {
    return {
      ...
      responsible_person_1: DEMO_PEOPLE_[index % DEMO_PEOPLE_.length],
      responsible_person_2: '',
      responsible_person_3: '',
      reason_importance: demo.reason,
      objective_goal: demo.objective,
      principle_theory: demo.principle,
      development_process: demo.process,
      success_evidence: demo.success,
      future_direction: demo.future,
      recognition_award: demo.award ? 'นำเสนอในเวที KM YRU Forum และได้รับ' + demo.award : '',
      knowledge_capture: 'สังเคราะห์เป็นแนวปฏิบัติที่ดี (Best Practice) สำหรับหน่วยงานอื่นนำไปประยุกต์ใช้ต่อ',
      attachment_file_id: '',
